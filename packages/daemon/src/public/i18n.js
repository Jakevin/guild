...
var I18N_ROWS = [
  ...
  ["invite.save", "加入", "Add"],
  ["assign.bar", "派工", "Dispatch"],
  ["assign.then", "然後", "then"],
  ["assign.title", "要誰做？", "Who should do this?"],
  ["assign.hint", "這則提到好幾個人。選一個去跑。", "Several people are named here. Pick one to run."],
  ["dispatch", "派工", "Assign"],
  ["dispatch.none", "這則沒有可派的人。", "Nobody to assign on this message."],
  ["dispatch.ok", "已派給 {who}", "Assigned to {who}"],
  ["cancel", "取消", "Cancel"],
  ["save", "儲存", "Save"],
  ...
  ["stats.messageId", "訊息", "Message"],
  ["askAgain", "重新詢問", "Ask again"],
  ["send", "送出", "Send"],
  ["jumpBottom", "滾到最新", "Jump to latest"],
  ["promptRail", "你發過的位置", "Your prompts"],
  ["promptJump", "你的訊息", "Your message"],
  ["stop", "停止", "Stop"],
  ...
  ["live.computer", "要操控這台 Mac", "Wants to use this Mac"],
  ["live.allow", "允許", "Allow"],
  ["live.deny", "不允許", "Don't allow"],
  ["steer.queue", "排隊", "Queue"],
  ["steer.insert", "插入引導", "Steer"],
  ["steer.remove", "取消排隊", "Remove from queue"],
  ["steer.tag", "已插入這輪", "Inserted into this turn"],
  ["steer.tagTo", "已插入 {who} 這輪", "Inserted into {who}'s turn"],
  ["steer.inserted", "已插入", "Inserted for"],
  ["steer.waiting", "排隊中，這輪結束後送出", "Queued. Sends when this turn finishes."],
  ["steer.waitingTo", "排隊給 {who}，這輪結束後送出", "Queued for {who}. Sends when this turn finishes."],
  ...
  ["memory.botHelp", "這名冒險者學會的站立筆記。有用的回合後會自動更新，並標上日期。人寫的委託書與最新一句話優先。", "Standing notes this adventurer has learned. Auto-updated and dated. Channel.md and the latest message outrank them."],
  ["memory.channel", "據點 MEMORY.md", "Hall MEMORY.md"],
  ["memory.channelHelp", "這個據點共享的站立筆記。有用的回合後會自動更新，並標上日期。人寫的委託書與最新一句話優先。", "Shared notes for this hall. Auto-updated and dated. The written Channel.md and the latest message outrank them."],
  ["memory.tidy", "整理記憶", "Tidy memory"],
  ["memory.tidying", "整理中…", "Tidying…"],
  ["memory.tidyHelp", "請模型依 Memory → Plan → Act 整理筆記並寫回 MEMORY.md。需求欄是本回合指令。", "Ask the model to tidy notes (Memory → Plan → Act) and write MEMORY.md. The need field is this turn's directive."],
  ["memory.tidySaved", "已寫回 MEMORY.md", "Wrote MEMORY.md"],
  ["memory.ask", "需求", "Need"],
  ["memory.askPh", "刪除 v0.2.26 降版的記憶", "drop the v0.2.26 downgrade notes"],
  ["memory.tidyFailed", "整理失敗", "Tidy failed"],
  ["memory.tidyEmpty", "還沒有記憶可整理", "Nothing to tidy yet"],
  ["memory.log", "版本", "Version"],
  ["memory.logEmpty", "還沒有可還原的版本", "No snapshot yet"],
  ["memory.restore", "還原", "Restore"],
  ["memory.restoreHelp", "把 MEMORY.md 還原成選中的快照。現況會先存一版。", "Restore MEMORY.md from the selected snapshot. The current file is snapshotted first."],
  ["memory.restored", "已還原 MEMORY.md", "Restored MEMORY.md"],
  ["memory.restoreFailed", "還原失敗", "Restore failed"],
  ["memory.needs", "還缺", "Still open"],
  ["memory.dropped", "已收進 Closed", "Moved to Closed"],
  ["bot.message", "密談", "Whisper"],
  ["bot.bare", "無痕密談", "Incognito"],
  ["bot.bareHelp", "不含 MEMORY.md。用來試 Soul、Agent、Position 與 harness。", "No MEMORY.md. Probe Soul, Agent, Position, and the harness."],
  ["bare.subtitle", "無痕 · 不含記憶", "Incognito · no memory"],
  ["bare.clear", "清除", "Clear"],
  ["bare.clearHelp", "清掉這次無痕紀錄。Soul、Agent、Position 和一般密談不會動。", "Wipe this incognito log. Soul, Agent, Position, and the regular whisper stay."],
  ["bare.clearConfirm", "清掉這次無痕密談？紀錄會刪掉。一般密談與 MEMORY.md 不會動。", "Clear this incognito whisper? The log goes. The regular whisper and MEMORY.md stay."],
  ["bare.cleared", "已清除無痕紀錄", "Incognito log cleared"],
  ...
  ["cron.jobTitle", "例行任務", "Scheduled task"],
  ["cron.enabled", "啟用中", "On"],
  ["cron.pausedLabel", "已暫停", "Paused"],
  ["cron.test", "測試執行", "Test run"],
  ["cron.name", "名稱", "Name"],
  ["cron.prompt", "指示", "Instructions"],
  ["cron.when", "執行時機", "Schedule"],
  ["cron.history", "執行歷史紀錄", "Run history"],
  ["cron.historyEmpty", "還沒有執行紀錄。", "No runs yet."],
  ["cron.justNow", "剛剛", "Just now"],
  ["cron.minsAgo", "{n} 分鐘前", "{n} min ago"],
  ["cron.hoursAgo", "{n} 小時前", "{n} hr ago"],
  ["cron.back", "返回", "Back"],
  ["cron.deleteConfirm", "刪除這個排程？", "Delete this schedule?"],
  ["cron.saveFailed", "排程沒寫進去", "Couldn't save the schedule"],
  ["cron.ok", "成功", "OK"],
  ["cron.failed", "失敗", "Failed"],
  ["cron.skipped", "略過", "Skipped"],
  ["cron.scope", "種類", "Kind"],
  ["cron.scopeChannel", "委託", "Quest"],
  ["cron.scopeBot", "獨立", "Independent"],
  ["cron.delivery", "回應", "Replies"],
  ["cron.deliveryHall", "混進聊天室", "Into the chat"],
  ["cron.deliverySheet", "只在排程頁", "Schedule page only"],
  ["cron.log", "這一輪", "This run"],
  ["cron.logEmpty", "還沒有隔離回覆。", "No isolated replies yet."],
  ["cron.create", "新增", "Add"],
  ["cron.whenPh", "每1小時", "every 1h"],
  ...
  ["settings.lede", "第一次：先連訂閱或填 API key，再套用主模型。沒有模型就不能用。訂閱走 OAuth；API key 寫入 ~/.guild/models.json。", "First time: connect a subscription or paste an API key, then apply a default. Guild does not run without a model. Subscriptions use OAuth. API keys go in ~/.guild/models.json."],
  ["settings.accounts", "連接帳號", "Accounts"],
  ["settings.accountsHint", "點名稱切換訂閱。勾選的模型會出現在大廳；主模型仍在「主模型」頁套用。", "Tap a name to switch. Checked models appear in the hall picker; the default is still applied on Main."],
  ...
  ["settings.efforts", "推理檔", "Reasoning levels"],
  ["settings.effortsPlaceholder", "low, medium, high", "low, medium, high"],
  ["settings.effortsHint", "catalog 沒有時才填，逗號分隔。", "Only if the catalog has none. Comma-separated."],
  ["settings.effortsCatalog", "來自 models.dev", "From models.dev"],
  ...
  ["settings.commandCode", "Command Code", "Command Code"],
  ["settings.commandCodeHint", "用 Command Code 訂閱當模型。模型清單跟官方 cmd --list-models 同一份（GET /provider/v1/models）。Go 方案沒有 Provider API，對話會走 /alpha/generate；方案沒有的模型伺服器會拒。Connect 走官方 cmd login，也可貼 user_ key。", "Use a Command Code subscription as the model. The picker is the official cmd --list-models catalog (GET /provider/v1/models). Go has no Provider API, so chat uses /alpha/generate; out-of-plan models are refused by the server. Connect uses official cmd login, or paste a user_ key."],
  ["settings.commandCodeLoginHint", "社群接線，條款是 Command Code 的。登入需要本機瀏覽器能打回 127.0.0.1。遠端 Host 請改貼 key。", "Community integration; Command Code's terms apply. Sign-in needs a browser that can reach 127.0.0.1. On a remote host, paste the key instead."],
  ["settings.commandCodeLoginOpened", "已開啟 Command Code 官方登入頁。", "Opened the official Command Code login page."],
  ["settings.antigravity", "Antigravity", "Antigravity"],
  ["settings.antigravityHint", "本機 agy CLI。模型清單走 agy models，只收 Gemini 3.8 Flash 以上。這一席由 agy 自己跑工具，不走 Guild 工具迴圈。沒有內建登入頁，請在終端機跑 agy auth。", "Local agy CLI. Catalog from agy models, Gemini 3.8 Flash and newer. This seat lets agy run its own tools; Guild's tool loop does not. No in-app login — run agy auth in a terminal."],
  ["settings.antigravityLoginHint", "先安裝 Antigravity CLI（agy），再在終端機執行 agy auth。Guild 只偵測 PATH 上的 agy（或 AGY_PATH）。", "Install the Antigravity CLI (agy), then run agy auth in a terminal. Guild only looks for agy on PATH (or AGY_PATH)."],
  ["settings.accountModels", "大廳要出現的模型", "Models in the hall picker"],
  ["settings.selectAll", "全選", "Select all"],
  ["settings.freebuffChat", "Freebuff Chat", "Freebuff Chat"],
  ["settings.freebuffChatHint", "用 Freebuff 官方免費 session 當模型（@codebuff/sdk，costMode=free）。Guild 大廳仍跑工具。連接會走官方裝置登入，憑證寫入 ~/.config/manicode/credentials.json（可與 Codex 共用）。摘要 / spawn / Studio 生成不會進這個分頁。", "Use Freebuff's official free session as the model (@codebuff/sdk, costMode=free). Guild still runs tools. Connect uses official device login and writes ~/.config/manicode/credentials.json (shared with Codex). Summaries / spawn / Studio generate skip this tab."],
  ["settings.freebuffLoginHint", "廣告、地區 limited-mode、服務條款都由 Freebuff 決定。不要在不信任的機器登入。", "Ads, regional limited-mode, and terms are Freebuff's. Do not sign in on a machine you do not trust."],
  ["settings.doctor", "檢查", "Check"],
  ["settings.freebuffDoctorFail", "探針失敗：{names}", "Probe failed: {names}"],
  ["settings.webLoginOpened", "已開啟 Freebuff 官方登入頁，請完成 GitHub/Google 登入。", "Opened the official Freebuff login page. Finish GitHub/Google sign-in there."],
  ["live.freebuffWait", "Freebuff：等待官方 session…", "Freebuff: waiting for official session…"],
  ["live.freebuffQueue", "Freebuff：排隊中…", "Freebuff: in queue…"],
  ["live.freebuffPaste", "Freebuff：準備提示詞…", "Freebuff: preparing prompt…"],
  ["live.freebuffSend", "Freebuff：呼叫 SDK…", "Freebuff: calling SDK…"],
  ["error.freebuff_login_required", "尚未登入官方 Freebuff session，或工作階段已過期。請到模型頁連接。", "Not signed in to the official Freebuff session, or it expired. Connect on Models."],
  ["error.freebuff_no_browser", "無法開啟官方登入頁。請手動開啟回傳的網址。", "Could not open the official login page. Open the returned URL yourself."],
  ["error.freebuff_ui_drift", "網頁介面已變更，無法繼續。", "The page UI changed. The bridge stopped."],
  ["error.freebuff_waiting_room", "排隊沒有進展。", "The queue did not move."],
  ["error.freebuff_ad_unresolved", "廣告擋住輸入框。", "An ad is blocking the composer."],
  ["error.freebuff_limited_mode", "目前地區沒有這個模型，或官方免費 session 不收這個模型。", "That model is not available here, or the official free session does not admit it."],
  ["error.freebuff_session_cap", "今日或本次工作階段額度用盡。", "Daily or session quota is used up."],
  ["error.freebuff_composer_rejected", "無法送出這則提示。", "Could not send this prompt."],
  ["error.freebuff_context_too_large", "內容超過 Freebuff 輸入上限。", "The prompt exceeds the Freebuff input limit."],
  ["error.freebuff_tool_parse", "工具呼叫格式無法解析。", "Could not parse the tool call."],
  ["error.freebuff_remote_agent", "網頁端代理關不掉。", "The remote page agent could not be turned off."],
  ["error.freebuff_stream_idle", "太久沒有可見進度。", "No visible progress for too long."],
  ["error.freebuff_busy", "另一個回合正在使用 Freebuff。", "Another turn is using Freebuff."],
  ["error.freebuff_disabled", "Freebuff 已停用。", "Freebuff is disabled."],
  ["error.freebuff_role_unsupported", "這個用途不能走 Freebuff，也沒有其他可用模型。", "This task cannot use Freebuff, and no other model is ready."],
  ["error.freebuff_window_closed", "瀏覽器視窗已關閉。", "The browser window was closed."],
  ["error.freebuff_unreachable_dispatch", "內部路徑錯誤，沒有對官方 session 送出請求。", "Internal path error; nothing was sent to the official session."],
  ...
  ["studio.listingTitle", "從說明頁起草", "Draft from an info page"],
  ["studio.listingHint", "貼 Grok Bot 市集的說明頁，例如 https://x.ai/bot/marketplace/bots/pg。會填入名字與 Soul / Agent / Position 草稿。不會複製技能、排程或 MCP。入座前請你改。", "Paste a Grok Bot marketplace info page, e.g. https://x.ai/bot/marketplace/bots/pg. Fills name plus Soul / Agent / Position drafts. Does not copy skills, schedules, or MCP. Edit before you seat them."],
  ["studio.listingUrl", "說明頁網址", "Info page URL"],
  ["studio.listingPh", "https://x.ai/bot/marketplace/bots/pg", "https://x.ai/bot/marketplace/bots/pg"],
  ["studio.listingDraft", "起草", "Draft"],
  ["studio.listingBusy", "讀取說明頁…", "Reading the info page…"],
  ["studio.listingNeedUrl", "先貼說明頁網址", "Paste an info page URL first"],
  ["studio.listingDone", "已填入草稿。技能、排程與 MCP 沒有複製。請改完再請他入座。", "Draft filled. Skills, schedules, and MCP were not copied. Edit, then seat them."],
  ["studio.listingThin", "這頁只有名字與一句話。完整職稱在市集說明頁。", "This page only has a name and one-liner. The full brief is on the marketplace info page."],
  ...
  ["lang.zh", "中", "中"],
  ["lang.en", "EN", "EN"],
  ["nav.menu", "委託與密談", "Channels and whispers"],
