...
test("hostRead refuses guild secrets before touching the disk", () => {
  ...
  refused(() => hostRead(join(homedir(), ".guild", "commandcode.json")));
  refused(() => hostRead(join(homedir(), ".guild", "commandcode-models.json")));
...
test("hostRead refuses credential directories under $HOME", () => {
  ...
  refused(() => hostRead(join(homedir(), ".commandcode", "auth.json")));
  refused(() => hostList(join(homedir(), ".commandcode")));
