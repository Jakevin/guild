...
CI (`.github/workflows/release.yml`) tests, `npm pack`s the daemon, then publishes that tarball so the registry manifest matches the packed `file:./vendor/protocol` (npm's directory publish would send `workspace:*` after `postpack` and break `npx`). It waits until `npm view` and the tarball match the tag, then creates the GitHub Release. A Release existing means that version is on npm. If publish looks successful but the registry never serves the tarball, the job fails — bump the version, do not reuse the tag. Catch-up a tag that already has a GitHub Release: Actions → Release → Run workflow → tag `v0.2.13`.
